...
class TestWithdrawal(SetupUserAndAccountMixin):
    ...
    def test_withdrawal_request(self):
        ...
        self.assertTrue(transaction.to_user.username in response.content.decode('utf8'))
        self.assertTrue(transaction.amount in response.content.decode('utf8'))
        response = self.client.post(post_url, data={'id': transaction.id}, follow=True)
        self.assertFalse(transaction.to_user.username in response.content.decode('utf8'))
        self.assertFalse(transaction.amount in response.content.decode('utf8'))
    ...
    def navigate_to_withdrawal_approval_page(self):
        ...
        self.assertEqual(response.status_code, 200)
        return '/accounts/withdrawals_approval/', response
...
@tag('ui')
class SeleniumWithdrawalTestCase(TestWithdrawal, LiveServerTestCase):
